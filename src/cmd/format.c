    ...
    .mkfs_parm =
        {
            ...
            FM_SFD：创建 “小软盘格式”（Small Floppy Disk，针对极小容量设备，如早期软盘，通常
            <16MB，会自动选择 FAT12）。
            */
            .fmt = FM_FAT,
            .n_fat = 0,  // FAT 表数量：若设为 0，FatFs 会自动使用默认值（通常为 2 个副本，主 FAT +
                         // 备份 FAT，提高可靠性）
            .align = 0,  // 数据区（Data Area）的起始对齐扇区数（单位：扇区，1 扇区通常为 512
                         // 字节）：若设为 0，FatFs 会自动选择最优对齐值（通常对齐到 1
                         // 扇区，即不额外偏移）
            .n_root = 0,  // 根目录数量：若设为 0，FatFs 会根据文件系统类型自动分配默认值（推荐）
            .au_size =
                0,  // 簇字节数：若设为 0，FatFs 会根据存储介质总容量自动选择最优簇大小（推荐）
        },
};
...
cmd_args_t cmd_parse_format_args(int argc, char** argv)
{
    ...
    *args = default_args;
    ...
    static struct option long_options[] = {
        {"img-path", required_argument, 0, 'p'}, {"fmt", optional_argument, 0, 'f'},
        {"n-fat", optional_argument, 0, 4},      {"align", optional_argument, 0, 5},
        {"n-root", optional_argument, 0, 6},     {"au-size", optional_argument, 0, 7},
